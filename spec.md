...
- **FR-04a: 複数シーン**: 異なるビジュアルパターンを「シーン」として複数定義できること。詳細は「7. 実装済みビジュアルシーン」を参照。
- **FR-04b: 自動遷移**: 設定された時間が経過すると、現在のシーンから次のシーンへ**なめらかに（クロスフェード等で）自動遷移**すること。
- **FR-04c: 手動切り替え**: キーボードの数字キー（`1`, `2`, `3`）で対応するシーンに即時切り替えが可能であること。
...
- **UIコントロール**: **Tweakpane** または **dat.GUI** (未実装)
