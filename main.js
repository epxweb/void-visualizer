...
let fgColorPicker, bgColorPicker;
let fgBaseColor = new THREE.Color();
let bgBaseColor = new THREE.Color();
let fgBaseHSL = { h: 0, s: 0, l: 0 };
let bgBaseHSL = { h: 0, s: 0, l: 0 };
// --- ここまで ---
...
const params = {
  ...
  visual: { grain: 0.1, backgroundColor: '#000000', foregroundColor: '#ffffff', autoshiftFgHue: false, autoshiftBgHue: false },
...
const setupUI = () => {
  ...
  // --- Autoshift Hue 機能 ---
  fgColorPicker = visualFolder.addBinding(params.visual, 'foregroundColor');
  fgColorPicker.on('change', (ev) => {
    sceneManager.updateForegroundColor(new THREE.Color(ev.value));
  });
  visualFolder.addBinding(params.visual, 'autoshiftFgHue', { label: 'Autoshift FG Hue' })
    .on('change', (ev) => {
      fgColorPicker.disabled = ev.value;
      if (ev.value) {
        fgBaseColor.set(params.visual.foregroundColor);
        fgBaseColor.getHSL(fgBaseHSL);
      }
    });

  bgColorPicker = visualFolder.addBinding(params.visual, 'backgroundColor');
  bgColorPicker.on('change', (ev) => {
    renderer.setClearColor(new THREE.Color(ev.value), params.visual.backgroundColor === 'transparent' ? 0 : 1);
  });
  visualFolder.addBinding(params.visual, 'autoshiftBgHue', { label: 'Autoshift BG Hue' })
    .on('change', (ev) => {
      bgColorPicker.disabled = ev.value;
      if (ev.value) {
        bgBaseColor.set(params.visual.backgroundColor);
        bgBaseColor.getHSL(bgBaseHSL);
      }
    });
  // --- ここまで ---

  visualFolder.addBinding(params.strobe, 'enable', { label: 'Strobe' });  visualFolder.addBinding(params.strobe, 'sensitivity', { label: 'Strobe Sensitivity', min: 0, max: 1.0, step: 0.05 });
...
const renderFrame = () => {
  ...
  // --- Autoshift Hue 更新ロジック ---
  if (params.visual.autoshiftFgHue) {
    const hue = (elapsedTime / 60) % 1.0;
    const newColor = new THREE.Color();
    newColor.setHSL(hue, fgBaseHSL.s, fgBaseHSL.l);
    
    params.visual.foregroundColor = '#' + newColor.getHexString();
    sceneManager.updateForegroundColor(newColor);
    if (fgColorPicker) fgColorPicker.refresh();
  }
  if (params.visual.autoshiftBgHue) {
    const hue = (elapsedTime / 60) % 1.0;
    const newColor = new THREE.Color();
    newColor.setHSL(hue, bgBaseHSL.s, bgBaseHSL.l);

    params.visual.backgroundColor = '#' + newColor.getHexString();
    renderer.setClearColor(newColor, params.visual.backgroundColor === 'transparent' ? 0 : 1);
    if (bgColorPicker) bgColorPicker.refresh();
  }
  // --- ここまで ---
